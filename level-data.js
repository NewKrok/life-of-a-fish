...
const LEVELS = [
  // ── Level 1: Coral Reef ──
  {
    ...
    switchGateGroups: [
      { id: 0, switches: [{ row: 8, col: 44 }], gates: [{ row: 12, col: 46 }] },
      { id: 2, switches: [{ row: 4, col: 62 }], gates: [{ row: 7, col: 62 }] },
      {
        id: 7,
  ...
  for (let row = 0; row < LEVEL_ROWS; row++) {
    for (let col = 0; col < LEVEL_COLS; col++) {
      ...
      if (t === 7) {
        ...
        entities.breakableWalls.push({ x: cx, y: cy });
        TILES[row][col] = 0;
      } else if (t === 28) {
        entities.armoredFish.push({ x: cx, y: cy });
        TILES[row][col] = 0;
      } else if (t === 29) {
  ...
  if (level.switchGateGroups) {
    for (const grp of level.switchGateGroups) {
      for (const s of grp.switches) {
        ...
        const match = allSwitches.find(
          (sw) => Math.abs(sw.x - sx) < 2 && Math.abs(sw.y - sy) < 2,
        );
      ...
      for (const g of grp.gates) {
        ...
        const match = entities.gates.find(
          (gt) => Math.abs(gt.x - gx) < 2 && Math.abs(gt.y - gy) < 2,
        );
        if (match) match.group = grp.id;
      }
    }
...
export function getMergedSolidBodies() {
  ...
  const bodies = [];
  const visited = Array.from({ length: LEVEL_ROWS }, () =>
    new Array(LEVEL_COLS).fill(false),
  );
